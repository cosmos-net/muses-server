...
import { createProjectGenerator } from '@test/utils/generators/project.generator';
...
describe('Get list module test persistence (e2e)', () => {
  ...
  describe('Given we want to get a sub-module', () => {
    describe('When exist sub-modules in db', () => {
      test('Then expect to return a 200 status code', async () => {
        ...
        const response = await request(app.getHttpServer()).get('/sub-module/list');
