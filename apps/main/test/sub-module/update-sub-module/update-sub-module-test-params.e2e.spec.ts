  ...
  describe('Given we want to update a sub-module', () => {
    describe('When send id with invalid', () => {
      test('Then expect a throw Bad Request exception', async () => {
        ...
        const response = await request(app.getHttpServer()).patch('/sub-module/').send(params);
    ...
    describe('When send sub-module to update but the db is empty', () => {
      test('Then expect a throw Bad Request exception', async () => {
        const params = {
          id: new ObjectId().toHexString(),
          name: faker.string.alpha(10),
